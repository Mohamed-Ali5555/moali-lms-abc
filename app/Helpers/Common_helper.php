...
if (!function_exists('lesson_count')) {
    ...
    {
        if ($course_id != '') {
            $total_lesson = DB::table('lessons')->whereStatus(1)->where('course_id', $course_id)->count();
            return $total_lesson;
        }
    }
}
...
if (!function_exists('currency')) {
    function currency($price = false)
    {
        ...
        $currency = $symbol . number_format(max(0, $price), 2);
...
if (!function_exists('viewLesson')) {
    function viewLesson($courseId, $lessonId)
    {
        ...
        $watchHistory = App\Models\Watch_history::where('course_id', $courseId)->where('student_id', $studentId)->first();

        $lesson = App\Models\Lesson::find($lessonId);
        if (!$lesson) return false;

        $section = App\Models\Section::where('course_id', $courseId)->orderBy('sort')->first();
        ...
        if ($watchHistory) {
            ...
            $sortedSections = App\Models\Section::where('course_id', $lesson->course_id)->orderBy('sort')->get();

            $nextLessonToWatch = null;

            foreach ($sortedSections as $section) {
                $sortedLessons = App\Models\Lesson::where('section_id', $section->id)->orderBy('sort')->get();
                foreach ($sortedLessons as $sortedLesson) {
                    if (!in_array($sortedLesson->id, $completedLessons)) {
                        $nextLessonToWatch = $sortedLesson;
                        break 2;
                    }
                }
            }

            if ($nextLessonToWatch && $nextLessonToWatch->id == $lessonId) {
                return true;
            }
        } else {
            $firstLesson = App\Models\Lesson::where('section_id', $section->id)->orderBy('sort')->first();
...
if (!function_exists('get_theme_settings')) {
    ...
    {
        $value =  Modules\Theme\App\Models\theme_setting::where('type', $type);
        if ($value->count() > 0) {
            if ($return_type === true) {
                return json_decode($value->value('description'), true);
            } elseif ($return_type === "object") {
...
if (!function_exists('get_locked_lesson_ids')) {
    function get_locked_lesson_ids($courseId, $userId) {
        ...
        foreach ($sections as $index => $section) {
            $lessons = App\Models\Lesson::whereStatus(1)->active()->where('section_id', $section->id)
                ->orderBy('sort')
                ->get();
