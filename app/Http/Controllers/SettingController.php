    ...
    public function payment_settings_update(Request $request)
    {
        ...
        if ($request->top_part == 'top_part') {
            ...
            if ($request->identifier == 'paypal') {
                ...
                $data['keys'] = $paypal;
            } elseif ($request->identifier == 'fawrypay') {
                $fawrypay = [
                    'merchant_code'       => $data['merchant_code'],
                    'secure_key'          => $data['secure_key'],
                    'merchant_code_test'  => $data['merchant_code_test'],
                    'secure_key_test'     => $data['secure_key_test'],
                    'expiry_in_hours'     => $data['expiry_in_hours'],
                ];
                $fawrypay     = json_encode($fawrypay);
                $data         = array_splice($data, 0, 4);
                $data['keys'] = $fawrypay;
            } elseif ($request->identifier == 'paymob') {
                $paymob = [
