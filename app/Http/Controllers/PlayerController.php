...
class PlayerController extends Controller
{
    public function course_player(Request $request, $slug, $id = '')
    {
        ...
        if(!$course){
                Session::flash('error', get_phrase('course are not found or not active.'));
        ...
        $first_lesson_of_course = Lesson::where('course_id', $course->id)->Active()->orderBy('sort', 'asc')->value('id');
        ...
        $page_data['lesson_details'] = Lesson::where('id', $id)->Active()->firstOrNew();
    ...
    public function set_watch_history(Request $request)
    {
        ...
        $total_lesson = Lesson::where('course_id', $request->course_id)->Active()->pluck('id')->toArray();
