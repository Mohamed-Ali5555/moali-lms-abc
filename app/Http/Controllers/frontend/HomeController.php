...
class HomeController extends Controller
{
    ...
    public function update_watch_history_with_duration(Request $request)
    {
        try {
            ...
            // Validate input
            if (!$courseId || !$lessonId || !is_numeric($currentDuration)) {
                return response()->json([
       ...
       if (
                $dripContentSettings['lesson_completion_role'] == 'duration' &&
                isset($dripContentSettings['minimum_duration']) &&
                $dripContentSettings['minimum_duration'] == 0
            ) {
                $isCompleted = 1;
            } else {
        ...
        if ($isCompleted == 1) {
            ...
            \Log::error('Error in update_watch_history_with_duration: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());

            return response()->json([
                'error' => 'An error occurred while updating watch history',
