...
class ApiController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $user = User::where('email', $email)->orWhere('phone',$phone)->where('status', 1)->first();

        if($user && Hash::check($fields['password'],$user->password) == true){
    ...
    public function category_subcategory_wise_course(Request $request)
    {
        $category_id = $request->category_id;
        $courses = get_category_wise_courses($category_id);
        $sub = Category::where('parent_id', $category_id)->where('status', 'active')->get();
