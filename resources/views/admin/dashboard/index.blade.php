...
@section('content')
    <div class="ol-card radius-8px">
        <div class="ol-card-body my-3 py-4 px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <h4 class="title fs-16px">
                    {{ get_phrase('الرصيد') }}
                    {{ \App\Models\Payment_history::sum('admin_revenue') + \App\Models\BootcampPurchase::sum('admin_revenue') }}
                </h4>
            </div>
        </div>
    </div>
    ...
    <div class="row g-2 g-sm-3 my-3 row-cols-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5">
        ...
        <div class="col">
            <div class="ol-card card-hover">
                <div class="ol-card-body px-20px py-3">
                    <p class="title card-title-hover fs-18px my-2">{{App\Models\Lesson::where('lesson_type','!=','quiz')->count()}}</p>
                    <p class="sub-title fs-14px">{{ get_phrase('Number of Lessons') }}</p>
                </div>
            </div>
        </div>
        <a href="{{route('admin.exams.list')}}">
            <div class="col">
                <div class="ol-card card-hover">
                    <div class="ol-card-body px-20px py-3">
                        <p class="title card-title-hover fs-18px my-2">{{App\Models\Lesson::where(['lesson_type'=>'quiz','type'=>'1'])->count()}}</p>
                        <p class="sub-title fs-14px">{{ get_phrase('Number of exams') }}</p>
                    </div>
                </div>
            </div>
        </a>
         <a href="{{route('admin.assignments.list')}}">
            <div class="col">
                <div class="ol-card card-hover">
                    <div class="ol-card-body px-20px py-3">
                        <p class="title card-title-hover fs-18px my-2">{{App\Models\Lesson::where(['lesson_type'=>'quiz','type'=>'2'])->count()}}</p>
                        <p class="sub-title fs-14px">{{ get_phrase('Number of assingment') }}</p>
                    </div>
                </div>
            </div>
        </a>
        <a href="{{route('admin.enroll.history')}}">
        <div class="col">
            <div class="ol-card card-hover">
                <div class="ol-card-body px-20px py-3">
