        ...
        <div class="ol-card-body my-3 py-4 px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                <h4 class="title fs-16px">
                    ...
                    {{ get_phrase('Purchase History book') }}
                </h4>
            </div>
        </div>
    </div>
    ...
    <div class="ol-card p-4">
        <div class="ol-card-body">
            <div class="row row-gap-3">
                @if (has_permission('admin.purchase.history.book.export'))
                ...
                @endif
                @if (has_permission('admin.purchase.history.book.search'))

                    <div class="col-md-6">
                        <form class="form-inline" action="{{ route('admin.purchase.history_book') }}" method="get">
                            <div class="row row-gap-3">
                                <div class="col-md-9">
                            ...
                            <table class="table eTable eTable-2 print-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">{{ get_phrase('Paid') }}</th>
                                ...
                                <tbody>
                                    @foreach ($reports as $key => $report)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                                    <div class="dAdmin_profile_img">
                                                        <img class="img-fluid rounded-circle image-45" width="45" height="45" src="{{ get_image($report->users->photo ?? '') }}" />
                                                    </div>
                                                    <div class="ms-1">
                                            ...
                                            <td>
                                                <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                                    <div class="dAdmin_profile_name">
                                                        @foreach($report->items as $row1)
                                                            <h4 class="title fs-14px">{{ $row1->item->title ?? '' }}</h4>
                                                        @endforeach
                                            ...
                                            <td>
                                                    ...
                                                    <p>{{ currency($report->items->sum('amount')) }}</p>
                                            ...
                                            <td>
                                                <span class="badge @if($report->paid == '1') bg-success @else bg-warning @endif">
                                                    @if($report->paid == '1') تم الدفع @else قيد الانتظار   @endif
                                                </span>
                                            </td>
                                            ...
                                            <td class="print-d-none">
                                                @if (has_permission('admin.purchase.history.book.invoice'))
                                                    <div class="adminTable-action">
                                                        <a class="btn ol-btn-light ol-icon-btn" data-bs-toggle="tooltip" title="{{ get_phrase('Print') }}" href="{{ route('admin.purchase.history.invoice',['id'=>$report->id,'type'=>'book']) }}">
                                    ...
                                    <tr>
                                        ...
                                        <th></th>
                                        <th>{{ get_phrase('Total') }} :
                                            {{ currency(\App\Models\InvoceItems::where('productable_type', 'Modules\BookStore\App\Models\Book')->sum('amount')) }}
                                        </th>
                                        <th></th>
                                        <th></th>
                                        <th></th>
